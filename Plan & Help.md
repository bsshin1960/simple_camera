...
### v2.7 – 카메라 기본 밝기 1차 조정
- **기본 밝기 20% 감소**: 카메라 비디오 프리뷰의 기본 밝기 배율을 1.8x에서 1.44x로 낮춰 눈부심을 방지하고 가독성을 개선

### v2.8 – 카메라 기본 밝기 2차 추가 조정
- **기본 밝기 추가 20% 감소**: 기본 밝기 배율을 1.44x에서 1.15x로 한 번 더 낮춤 (기존 1.8x 대비 총 36% 감소)

### v2.9 – 카메라 기본 밝기 1.3 최종 최적화 (최신)
- **기본 밝기 1.3배 설정**: 자연스럽고 편안한 시각 처리를 위해 기본 밝기 배율을 1.3배로 변경
- **캐시 버스팅 및 버저닝**: `index.html`, `app.js`의 서비스 워커 등록 경로, `sw.js`의 `CACHE_NAME` 버전을 갱신하여 클라이언트 단에 즉각 반영되도록 처리
